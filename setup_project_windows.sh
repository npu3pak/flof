...
function create_self_signed_cert() {
    filename=$1
    hostname=$2
    echo "Создание сертификата $filename $hostname"
    echo "/CN=$hostname"
...
function main {
    ...
    for host in ${hosts[@]}; do
        filename="$(pwd)/$NGINX_CERTS_PATH/$host"
        create_self_signed_cert $filename $host
    ...
    echo "systemProp.javax.net.ssl.trustStore=drive_letter:\\full\\path\\to\\gradle-truststore.jks" >> $GRADLE_PROPERTIES_PATH
